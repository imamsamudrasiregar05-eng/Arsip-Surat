...
if (isset($_POST['addMasuk'])) {
    ...
    $asal_surat = $koneksi->real_escape_string($_POST['asal_surat']);
    ...
    if (!empty($_FILES['file_surat']['name'])) {
        ...
        if (in_array($ext, $allowed) && $f['size'] <= 2*1024*1024) {
            ...
            if (!is_dir(UPLOAD_MASUK)) mkdir(UPLOAD_MASUK, 0755, true);
            if (!move_uploaded_file($f['tmp_name'], UPLOAD_MASUK . $fileName)) {
...
$koneksi->query("INSERT INTO surat_masuk 
(no_surat, judul_surat, tgl_surat, jenis_surat, asal_surat, karyawan, file_surat) VALUES 
('$no','$judul','$tgl','$jenis_surat','$asal_surat','$karyawan','$fileName')");
...
if (isset($_GET['delete'])) {
    $id = (int)$_GET['delete'];
    $row = $koneksi->query("SELECT file_surat FROM surat_masuk WHERE id=$id")->fetch_assoc();
    if ($row && $row['file_surat']) {
        @unlink(UPLOAD_MASUK . $row['file_surat']);
    }
...
// === Ambil Data ===
$data = $koneksi->query("SELECT * FROM surat_masuk ORDER BY created_at DESC");
?>
...
<div class="card p-3">
  <table id="tblMasuk" class="table table-striped">
    <thead>
      <tr>
        <th>#</th>
        <th>No Surat</th>
      ...
      <tr>
        ...
        <td>
          <?php if($row['file_surat']): ?>
            <a href="download.php?type=masuk&file=<?php echo urlencode($row['file_surat']); ?>">Download</a>
            | <a href="<?php echo UPLOAD_MASUK_URL . $row['file_surat']; ?>" target="_blank">Lihat</a>
          <?php else: echo '-'; endif; ?>
        </td>
        <td>
...
<div class="mb-3">
  ...
  <select name="asal_surat" class="form-control" required>
...
<div class="mb-3">
  ...
  <input type="text" name="karyawan" class="form-control" placeholder="Masukkan nama karyawan" required>
      ...
      <div class="modal-footer">
        <button type="submit" name="addMasuk" class="btn" style="background:#0d47a1;color:#fff">Simpan</button>
      </div>
    </form>
  </div>
</div>
